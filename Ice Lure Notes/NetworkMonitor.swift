...
extension LoadingView {
    ...
    func req() {
        ...
        conversionData["bundle_id"] = "com.app.icelurenotess"
        conversionData["os"] = "iOS"
        conversionData["store_id"] = "6757925284"
        conversionData["locale"] = Locale.current.identifier
        conversionData["firebase_project_id"] = "1086850480682"

        do {
            let jsonData = try JSONSerialization.data(withJSONObject: conversionData, options: [])
                    let url = URL(string: "https://icelurrenotes.com/config.php")!
